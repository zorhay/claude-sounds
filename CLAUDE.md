...
**Narrator:** `narrate.py` receives hook event JSON on stdin, extracts context (file paths, commands, patterns), calls an LLM for a one-sentence commentary, speaks it via TTS. Supports 5 providers (claude_cli, anthropic, gemini, openai, ollama) — all via raw HTTP, no SDK dependencies. Lock file prevents overlapping narrations. Every style prompt is augmented with `AUDIO_CLARITY_RULES` so the LLM returns plain speakable prose (no markdown, no URLs, no code fences). CLI modes: normal (hook playback), `--check` (test provider), `--check-tts` (test TTS engine), `--speak "text"` (speak via configured engine), `--dry-run` (text only), `--deep` (force deep-context mode on for one invocation).

**Deep context mode:** Enabled by `narrator_deep_context` config flag (or `--deep` CLI flag). When on, each narration remembers prior moments from the same `session_id` (Claude Code hook field). Persisted in `session_context/<session_id>.json` as `{summary, recent: [{ctx, text}], event_count}`. Recent narrations are embedded into the user turn so the LLM can reference earlier work ("back to the auth file…"). When `recent` exceeds 10 entries, the oldest are folded into `summary` via a separate compression LLM call (uses `COMPRESS_SYSTEM` prompt), keeping the last 4 verbatim. Session files older than 7 days are pruned on each save. The `call_provider()` signature accepts an optional `session` param; `_dispatch()` is the low-level LLM call used by both narration and compression.
...
- Narrator config keys: `narrator_provider`, `narrator_model`, `narrator_api_key`, `narrator_style`, `narrator_deep_context`.
